...
static void account_fixups_fn(void *var)
{
	...
	{
		if(account->acosc_penalty_duration > 10) { account->acosc_penalty_duration = 10; }	//10 is max penalty duration for hidecaeds since biger value disconnect client which we no want to happen.
	}
}
#endif
