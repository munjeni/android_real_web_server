	...
	while(cl->thread_active)
	{
		...
		while(cl->thread_active)
		{
			...
			case ACTION_CLIENT_HIDECARDS:
			{
				if (config_enabled(MODULE_CCCAM))
				{
					hidetime = (cl->account->acosc_penalty_duration == -1 ? cfg.acosc_penalty_duration * 1000 : cl->account->acosc_penalty_duration * 1000);
					if(hidetime)
					{
						cc_srv_report_cards(cl, 1);
						cs_log("Hiding cards for %s for %d secconds", username(cl), hidetime / 1000);
					}
				}
				break;
			}
			case ACTION_CLIENT_UNHIDECARDS:
			{
				if (config_enabled(MODULE_CCCAM))
				{
					...
					if(hidetime)
					{
						...
						//cl->n_request[0] = 0;
						//cl->n_request[1] = 0;
						cc_srv_report_cards(cl, 0);
						cs_log("Unhiden cards for %s", username(cl));
					}
				}
				break;
			}
