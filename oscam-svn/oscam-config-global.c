...
static const struct config_list global_opts[] =
{
	...
	DEF_OPT_UINT8("maxecmtime"              , OFS(maxecmtime),           0),
	DEF_OPT_UINT8("maxecmtimenotfound"      , OFS(maxecmtimenotfound),   0),
...
static void anticasc_fixups_fn(void *UNUSED(var))
{
	...
	{
		if(cfg.acosc_penalty_duration > 10) { cfg.acosc_penalty_duration = 10; }	//10 is max penalty duration for hidecaeds since biger value disconnect client which we no want to happen.
	}
