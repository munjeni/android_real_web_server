	...
	if(cfg.acosc_enabled)
	{
	  ...
	  //**
		...
		if((er->rc < E_NOTFOUND && max_active_sids > 0) || zap_limit > 0 || max_ecms_per_minute > 0)
		{
			...
			if(client->account->acosc_penalty_active == 0 && max_ecms_per_minute > 0 && client->n_request[1] > max_ecms_per_minute) //max_ecms_per_minute reached (curently configured to count current ecm count aka n_request[1] instead of count ecms from last minute aka n_request[0])
			{
			...
			if(client->account->acosc_penalty_active > 0)
			{
				...
				switch(penalty)
				{
					...
					case 4: // hidecards
						if(client->account->acosc_penalty_active == 1)
							{ cs_log("[zaplist] ACoSC for Client: %s  max_activ_sids reached: %i:%i(%s) penalty: 4(%s) hidecards - hidecards to the client for %i sec", username(client), active_sid_count, max_active_sids, info1, info2, penalty_duration); }
						if(client->account->acosc_penalty_active == 2)
							{ cs_log("[zaplist] ACoSC for Client: %s  zap_limit reached: %i:%i(%s) penalty: 4(%s) hidecards - hidecards to the client for %i sec", username(client), client->account->acosc_user_zap_count, zap_limit, info5, info2, penalty_duration);	}
						if(client->account->acosc_penalty_active == 3)
							{ cs_log("[maxecms] ACoSC for Client: %s  max_ecms_per_minute reached: ecms_last_minute=%i ecms_now=%i max=%i(%s) penalty: 4(%s) hidecards - hidecards to the client for %i sec", username(client), client->n_request[0], client->n_request[1], max_ecms_per_minute, info6, info2, penalty_duration); }
						add_job(client, ACTION_CLIENT_HIDECARDS, NULL, 0);
						add_job(client, ACTION_CLIENT_UNHIDECARDS, NULL, 0);
						break;
					default: // logging
						if(client->account->acosc_penalty_active == 1)
