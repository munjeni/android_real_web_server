...
int blowfish_setup(unsigned char *key, int keylen, int num_rounds, union symmetric_key *skey)
{
   ...
   /* copy the key into the L array in little endian fashion */
   for (A = x = y = 0; x < keylen; ) {
       A = (A << 8) | ((unsigned long)key[x++]);
       if (!(x & 3)) {
          L[y++] = BSWAP(A);
          A = 0;
       }
   }
   /* if length is not a nice multiple of 32 bits add one to y */
   if (keylen & 3) { A <<= (8 * (3-(keylen&3))); L[y] = BSWAP(A); ++y; }

   /* copy and xor key bits into K array */
   for (x = 0; x < 18; x++)
	skey->blowfish.K[x] = ORIG_P[x] ^ L[x % y];
   ...
   for (x = 0; x < 18; x += 2) {
       ...
       /* copy it */
       LOAD32L(skey->blowfish.K[x], &B[0]);
       LOAD32L(skey->blowfish.K[x+1], &B[4]);
   }
   ...
   for (x = 0; x < 4; x++)
       for (y = 0; y < 256; y += 2) { 
          ...
          blowfish_ecb_encrypt(B, B, skey);
          /* copy it */
          LOAD32L(skey->blowfish.S[x][y], &B[0]);
          LOAD32L(skey->blowfish.S[x][y+1], &B[4]);
       }
...
void blowfish_ecb_encrypt(unsigned char *pt, unsigned char *ct, union symmetric_key *key)
{
   ...
   /* do 16 rounds */
   for (i = 0; i < 16; i += 2) {
       L ^= key->blowfish.K[i];   R ^= F(L, key);
       R ^= key->blowfish.K[i+1]; L ^= F(R, key);
   }
...
void blowfish_ecb_decrypt(unsigned char *ct, unsigned char *pt, union symmetric_key *key)
{
   ...
   /* do 16 rounds */
   for (i = 14; i >= 0; i -= 2) {
       L ^= F(R, key); R ^= key->blowfish.K[i+1];
       R ^= F(L, key); L ^= key->blowfish.K[i];
   }
...
int blowfish_test(void)
{
   ...
   /* encrypt and decrypt */
   blowfish_ecb_encrypt(pt, buf[0], &key);
