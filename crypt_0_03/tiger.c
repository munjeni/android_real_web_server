...
const static unsigned long long table[4*256] = {
...
static void tiger_compress(union hash_state *md)
{
    unsigned long long a, b, c, x[8];
    unsigned long i;

    /* load words */
    for (i = 0; i < 8; i++) LOAD64L(x[i],&md->tiger.buf[8*i]);
    a = md->tiger.state[0];
    b = md->tiger.state[1];
    c = md->tiger.state[2];
...
void tiger_init(union hash_state *md)
{
    md->tiger.state[0] = 0x0123456789ABCDEF;
    md->tiger.state[1] = 0xFEDCBA9876543210;
    md->tiger.state[2] = 0xF096A5B4C3B2E187;
...
void tiger_process(union hash_state * md, unsigned char *buf, int len)
{
    while (len--) {
        /* copy byte */
        md->tiger.buf[md->tiger.curlen++] = *buf++;
...
void tiger_done(union hash_state * md, unsigned char *hash)
{
    ...
    /* append length */
    for (i = 56; i < 60; i++)
        md->tiger.buf[i] = (md->tiger.length >> ((4 - (60 - i)) * 8)) & 255;
    tiger_compress(md);
