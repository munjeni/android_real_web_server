...
static void test_short_bug()
{
  ...
  uchar      tiny_value;
  char       llbuf[22];
  myheader("test_unsigned short_bug");
  ...
  stmt= mysql_simple_prepare(mysql, "SELECT * FROM test_unsigned short");
  check_stmt(stmt);
  ...
  if (!opt_silent)
  {
    fprintf(stdout, "\n unsigned short   : %d (%ld)", short_value, s_length);
...
static void test_truncation()
{
  ...
  /* short -> unsigned short (no truncation, data is in the range of target type) */
  DIE_UNLESS(my_bind++ < bind_array + bind_count);
  DIE_UNLESS(! *my_bind->error && * (uint16*) my_bind->buffer == 32000);

  /* unsigned short -> utiny */
  DIE_UNLESS(my_bind++ < bind_array + bind_count);
